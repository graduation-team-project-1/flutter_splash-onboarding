...
class CalendarTab extends StatefulWidget {
  const CalendarTab({super.key, required this.pregnancyProfileModel});
      ...
      body: Column(
        children: [
          TableCalendar(
            ...
            lastDay: DateTime.utc(2045, 10, 10),
              ...
              rangeHighlightBuilder: (context, day, isWithinRange) {
                if (isWithinRange) {
                  int? weekNumber = StringManager().calculatePregnancyDetails(
                      widget.pregnancyProfileModel.user?.pregnant?.dueDate ??
                          widget.pregnancyProfileModel.user?.pregnant
                              ?.firstDayOfLastPeriod ??
                          widget.pregnancyProfileModel.user?.pregnant
                              ?.dateOfConception ??
                          '')[1];
                  weekNumber = weekNumber == 1 ? weekNumber! - 1 : weekNumber;
                  weekNumber = weekNumber! >= 40 ? weekNumber = 39 : weekNumber;
